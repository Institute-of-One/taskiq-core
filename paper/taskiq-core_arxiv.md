...
The central identity — that NEQ determines ideal-observer detectability — was tested by computing d'^2 two independent ways for a disk signal imaged through a Gaussian system: (A) from the independently validated prewhitening observer applied to the imaged signal, and (B) by integrating NEQ = MTF^2/NPS against the *object* signal's power spectrum. Across nine conditions (contrast 3, 6, 9; system blur 0.10, 0.15, 0.20 mm) the two routes agreed to a maximum relative error of **4.4e-16** — machine precision (Figure 2). The agreement is algebraic rather than merely numerical: under the synthetic model and the implemented discrete conventions the two routes are the same integral rearranged, so on synthetic data with known truth the physics (NEQ) determines the task (d') exactly, and the computation confirms it to the resolution of floating-point arithmetic.
...
On swept data the framework recovers the transfer laws that theory fixes. Ideal-observer d'^2 is proportional to contrast^2, so a fit of d'^2 on contrast^2 through the origin returned a coefficient of determination numerically equal to 1 — its residual sum of squares fell below the floating-point resolution of the fit (Figure 3, left); d'^2 is proportional to inverse noise variance (the dose law), and a fit of d'^2 on 1/sigma^2 likewise returned a coefficient of determination indistinguishable from 1 (Figure 3, right). The efficiency of the non-prewhitening eye-filter observer relative to the ideal observer, (d'/d'_ideal)^2, was constant across contrast to eight significant figures (absolute spread 3.4e-9), as it must be for two linear observers, confirming that the sweep and regression carry the exactness of the underlying identities rather than blurring it.

![**Figure 3.** The transfer laws recovered from swept data. Left: ideal-observer d'^2 against contrast^2, with the through-origin fit (coefficient of determination indistinguishable from 1). Right: ideal-observer d'^2 against inverse noise variance 1/sigma^2, with the through-origin fit (coefficient of determination indistinguishable from 1).](figures/fig3_transfer.png)
